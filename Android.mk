...
ifeq ($(GET_FRAMEBUFFER_FORMAT_FROM_HWC),true)
    EXTRA_CXXFLAGS += -DGET_FRAMEBUFFER_FORMAT_FROM_HWC
endif
...
$(LOCAL_BUILT_MODULE): $(TARGET_CRTBEGIN_DYNAMIC_O) $(TARGET_CRTEND_O) $(addprefix $(TARGET_OUT_SHARED_LIBRARIES)/,$(GECKO_LIB_DEPS)) $(GECKO_LIB_STATIC)
	...
	echo "export HAVE_KAIOS_256MB_SUPPORT=$(HAVE_KAIOS_256MB_SUPPORT)"; \
	...
	export HAVE_KAIOS_256MB_SUPPORT="$(HAVE_KAIOS_256MB_SUPPORT)" && \
	export HAVE_TEE_SUI_SUPPORT="$(HAVE_TEE_SUI_SUPPORT)" && \
	export HAVE_TOUCH_SUPPORT="$(HAVE_TOUCH_SUPPORT)" && \
	export KAI_FONT_CONFIG_FILE="$(KAI_FONT_CONFIG_FILE)" && \
	export ENABLE_EMBMS="$(TARGET_ENABLE_EMBMS)" && \
